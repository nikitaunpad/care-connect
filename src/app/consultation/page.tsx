    ...
    <div className="min-h-screen flex flex-col bg-[#F7F3ED] text-[#193C1F] font-sans">
      ...
      <header className="w-full h-20 px-12 flex items-center justify-between bg-white border-b border-[#D0D5CB]">
        ...
        <nav className="flex items-center gap-8">
          <Link href="/landingPage" className="text-[#193C1F] font-medium hover:text-[#8EA087]">Home</Link>
          <Link href="#" className="text-[#193C1F] font-medium hover:text-[#8EA087]">Resources</Link>
          <Link href="#" className="text-[#193C1F] font-medium hover:text-[#8EA087]">Support</Link>
        ...
        <div className="bg-white rounded-2xl border border-[#D0D5CB] p-12 max-w-[800px] w-full shadow-sm">
          ...
          <div className="text-center mb-10">
            <div className="w-14 h-14 bg-[#F7F3ED] rounded-full flex items-center justify-center mx-auto mb-6 border border-[#D0D5CB]">
              <svg className="h-6 w-6 text-[#8EA087]" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"></path>
              </svg>
            </div>
            <h1 className="text-3xl font-bold text-[#193C1F] mb-4">Consultation Form</h1>
          ...
          {/* Status Message */}
          {message.text && (
            <div className={`mb-6 p-4 rounded-xl border flex items-center gap-3 ${message.type === 'success' ? 'bg-[#8EA087]/10 text-[#193C1F] border-[#8EA087]/30' : 'bg-red-50 text-red-700 border-red-200'}`}>
              {message.type === 'success' ? (
                <svg className="h-5 w-5 text-[#8EA087]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
              ) : (
          ...
          <form className="space-y-8" onSubmit={handleSubmit}>
            <div>
              <label className="block text-sm font-semibold text-[#193C1F] mb-2" htmlFor="inquiry-title">Inquiry Title</label>
              <input name="title" required className="w-full px-4 py-4 rounded-xl border border-[#D0D5CB] bg-white text-[#193C1F] focus:ring-[#8EA087] focus:border-[#8EA087] placeholder-[#193C1F]/30" id="inquiry-title" placeholder="Enter a brief title for your request" type="text" />
            </div>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#193C1F] mb-2" htmlFor="detailed-description">Detailed Description</label>
              <textarea name="description" required className="w-full px-4 py-4 rounded-xl border border-[#D0D5CB] bg-white text-[#193C1F] focus:ring-[#8EA087] focus:border-[#8EA087] placeholder-[#193C1F]/30" id="detailed-description" placeholder="Please describe your situation here..." rows={5}></textarea>
            </div>
            ...
            <div className="bg-[#F7F3ED] border border-[#D0D5CB] rounded-xl px-6 py-5 flex items-center justify-between">
              <div>
                <h4 className="font-semibold text-[#193C1F]">Submit Anonymously</h4>
                <p className="text-xs text-[#193C1F]/60">Your identity will be hidden from the reviewer</p>
              </div>
              <div className="relative inline-block w-12 align-middle select-none transition duration-200 ease-in">
                <input className="peer absolute block w-6 h-6 rounded-full bg-white border-4 border-[#D0D5CB] appearance-none cursor-pointer checked:right-0 checked:border-[#8EA087]" id="toggle" name="isAnonymous" type="checkbox" />
                <label className="block overflow-hidden h-6 rounded-full bg-[#D0D5CB] cursor-pointer peer-checked:bg-[#8EA087] transition-colors duration-200" htmlFor="toggle"></label>
              </div>
            </div>

            <button disabled={isSubmitting} className="w-full py-5 bg-[#8EA087] hover:bg-[#8EA087]/90 disabled:opacity-50 disabled:cursor-not-allowed text-[#193C1F] font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-sm" type="submit">
              {isSubmitting ? (
            ...
            <div className="bg-white border border-[#D0D5CB]/30 rounded-xl p-4 flex items-start gap-3">
              <svg className="h-5 w-5 text-[#8EA087] mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <p className="text-sm text-[#193C1F]/70">
                If you are in immediate danger, please contact your local emergency services or use the <Link className="text-[#8EA087] font-semibold underline" href="#">Emergency</Link> shortcut in the navigation.
              </p>
            </div>
          </form>
        </div>
      ...
      <footer className="w-full py-12 px-12 bg-white border-t border-[#D0D5CB] mt-10">
        <div className="flex flex-col items-center gap-6">
          <div className="flex gap-8">
            <Link href="#" className="text-xs font-bold text-[#193C1F]/60 hover:text-[#193C1F] tracking-widest uppercase">Privacy Policy</Link>
            <Link href="#" className="text-xs font-bold text-[#193C1F]/60 hover:text-[#193C1F] tracking-widest uppercase">Terms of Service</Link>
            <Link href="#" className="text-xs font-bold text-[#193C1F]/60 hover:text-[#193C1F] tracking-widest uppercase">Help Center</Link>
          </div>
          <p className="text-sm text-[#193C1F]/40">© 2024 CareConnect. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
