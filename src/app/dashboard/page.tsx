...
export default async function DashboardPage({ searchParams }: { searchParams?: DashboardSearchParams }) {
  ...
  const searchBarQuery = rawSearch?.toLowerCase() || "";
  ...
  const totalDonationAmount = donations.reduce((accumulator, current) => accumulator + current.amount, 0);
  const pendingReportsCount = reports.filter((item) => item.status === "PENDING").length;

  const displayName = currentUser?.name?.trim() || session.user.name?.trim() || session.user.email.split("@")[0] || "there";
  return (
    <div className="p-12 space-y-10 animate-fade-in">
      ...
      <div className="flex gap-8">
        {[
          { label: "Total Consultations", val: consultations.length.toString(), icon: <ConsultationIcon /> },
          { label: "Reports Filed", val: reports.length.toString(), icon: <ReportsIcon /> },
          { label: "Donations Received", val: formatRupiah(totalDonationAmount), icon: <DonationsIcon /> },
        ].map((item, index) => (
          <div key={index} className="bg-[#F7F3ED] p-8 rounded-[28px] border border-[#D0D5CB] flex items-center gap-6 flex-1 shadow-sm">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
        <div className="bg-white border border-[#D0D5CB] rounded-[32px] overflow-hidden shadow-sm">
          ...
          <table className="w-full text-left">
            ...
            <tbody className="text-[14px] text-[#193C1F]">
              {recentConsultations.slice(0, 3).map((row) => (
                <tr key={row.id} className="border-b border-[#F7F3ED] hover:bg-[#FDFCFB]">
                  <td className="px-8 py-5 font-bold">{row.doctor}</td>
                  <td className="px-8 py-5 opacity-70">{row.dateLabel}</td>
                  <td className="px-8 py-5">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black ${row.status === "SCHEDULED" ? "bg-[#D1B698]/20 text-[#D1B698]" : "bg-[#EBE6DE] text-[#193C1F]"}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
          ...
          {recentConsultations.length === 0 && <p className="p-10 text-center text-[#8EA087]">No consultations found.</p>}
        </div>
            ...
            <tbody className="text-[14px] text-[#193C1F]">
              {filteredReports.slice(0, 3).map((row) => (
                <tr key={row.id} className="border-b border-[#F7F3ED] hover:bg-[#FDFCFB]">
                  ...
                  <td className="px-8 py-5">
                    <span className={`px-4 py-1.5 rounded-full text-[10px] font-black ${row.status === "PENDING" ? "bg-[#D1B698]/30 text-[#D1B698]" : "bg-[#EBE6DE] text-[#193C1F]"}`}>
                      {row.status}
                    </span>
                  </td>
                </tr>
